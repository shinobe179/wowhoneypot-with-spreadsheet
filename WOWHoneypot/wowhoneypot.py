...
class WOWHoneypotRequestHandler(BaseHTTPRequestHandler):
    ...
    def handle_one_request(self):
        ...
        try:
            ...
            rpath = ''
            if self.requestline:
                rmethod = self.requestline.split()[0]
                rpath = self.requestline.split()[1]

            request_all = self.requestline + "\n" + str(self.headers) + body
            logging_access("[{time}]{s}{clientip}{s}{hostname}{s}\"{requestline}\"{s}{status_code}{s}{match_result}{s}{requestall}{s}{ua}{s}{rmethod}{s}{rpath}\n".format(
                                                                    time=get_time(),
                                                                    clientip=clientip,
                                                                    hostname=hostname,
